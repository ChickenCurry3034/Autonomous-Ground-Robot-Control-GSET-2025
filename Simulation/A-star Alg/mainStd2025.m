...
end_node = 'Z';
tic
best_path = astar(nodes_map, connections_map, start_node, end_node);
...
function path = astar(nodes, connections, start_node, end_node)

    % Nested function for heuristic calculation
    function h_score = calculate_heuristic(node_name, goal_node_name)
        pos_node = nodes(node_name);
        pos_goal = nodes(goal_node_name);
        h_score = hypot(pos_goal(1) - pos_node(1), pos_goal(2) - pos_node(2));
    end
    ...
    for k = 1:length(node_names)
        g_score(node_names{k}) = inf;
        f_score(node_names{k}) = inf;
    end

    g_score(start_node) = 0;
    f_score(start_node) = calculate_heuristic(start_node, end_node);

    open_set = {start_node};
    closed_set = containers.Map('KeyType', 'char', 'ValueType', 'logical');
    ...
    while ~isempty(open_set)
        ...
        for k = 1:length(open_set)
            node = open_set{k};
            if f_score.isKey(node) && f_score(node) < min_f_score
        ...
        if isempty(current_node) || f_score(current_node) == inf
            path = {}; % No path found
            return;
        end
        ...
        if strcmp(current_node, end_node)
            % Path Reconstruction (integrated directly)
            path = {};
            current = end_node;
            ...
            % If the end_node has no predecessor, it means no path was found to it
            ...
            if strcmp(current, start_node)
                ...
                path = {}; % Should not happen if a path was found correctly
            end
            return; % Exit the function with the found path
        end
        ...
        if connections.isKey(current_node)
            ...
            for k = 1:length(neighbors)
                ...
                if closed_set.isKey(neighbor) && closed_set(neighbor)
                    continue;
                end
                ...
                tentative_g_score = g_score(current_node) + dist_to_neighbor;

                if tentative_g_score < g_score(neighbor)
                    prev(neighbor) = current_node;
                    g_score(neighbor) = tentative_g_score;
                    f_score(neighbor) = g_score(neighbor) + calculate_heuristic(neighbor, end_node);

                    if ~any(strcmp(open_set, neighbor))
    ...
    % If the loop finishes and the end_node was not reached, no path exists.
    path = {};
end
